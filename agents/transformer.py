...
def make_transformer(params, act=nn.LeakyReLU(), tail=[]):
    l = [params.emb_dim] + [params.n_hid] * params.n_layers + [params.n_words]
    mlp = make_mlp(l=l, act=act, tail=tail)
    model = TransformerModel(params, mlp)
    return model
