...
class DecentralizedPPO(BasePolicy):
    def __init__(self, env_info: dict):
        ...
        self.agents_name = env_info["agents_name"]
        # 初始化网络
        self.rnn_hidden_dim = 64
        actor_input_shape = env_info["obs_space"] * self.n_agents
        self.ppo_actors = {
            a: CentralizedPPOActor(
                actor_input_shape, self.action_dim, self.n_agents, self.rnn_hidden_dim
            )
            for a in self.agents_name
        }
        self.ppo_critics = {
            a: CentralizedPPOCritic(env_info["state_space"]) for a in self.agents_name
        }
        self.optimizer_actors = {
            a: torch.optim.Adam(
                params=self.ppo_actors[a].parameters(), lr=self.train_config.lr_actor
            )
            for a in self.agents_name
        }
        self.optimizer_critics = {
            a: torch.optim.Adam(
                params=self.ppo_critics[a].parameters(), lr=self.train_config.lr_critic
            )
            for a in self.agents_name
        }
        # 初始化路径
        ...
        self.ppo_actor_path = os.path.join(self.model_path, "ppo_actors.pth")
        self.ppo_critic_path = os.path.join(self.model_path, "ppo_critics.pth")
        ...
        for a in self.agents_name:
            self.ppo_actors[a].to(self.device)
            self.ppo_critics[a].to(self.device)
    ...
    def init_wight(self):
        for a in self.agents_name:
            self.ppo_actors[a].apply(weight_init)
            self.ppo_critics[a].apply(weight_init)
    ...
    def learn(self, batch_data: dict, episode_num: int):
        ...
        for i in range(self.train_config.learn_num):
            ...
            action_means, self.rnn_hidden = self.ppo_actor(
                obs, self.rnn_hidden.detach()
            )
    ...
    def save_model(self):
        torch.save(
            {a: self.ppo_actors[a].state_dict() for a in self.agents_name},
            self.ppo_actor_path,
        )
        torch.save(
            {a: self.ppo_critics[a].state_dict() for a in self.agents_name},
            self.ppo_critic_path,
        )

    def load_model(self):
        actors = torch.load(self.ppo_actor_path)
        critics = torch.load(self.ppo_critic_path)
        for a in self.agents_name:
            self.ppo_actors[a].load_state_dict(actors[a])
            self.ppo_critics[a].load_state_dict(critics[a])
