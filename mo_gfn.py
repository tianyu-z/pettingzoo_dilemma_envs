...
class FMGFlowNetGenerator(GeneratorBase):
    ...
    def preprocess_state(self, input_batch):
        ...
        if self.args.gen_model_type == "mlp":
            inp_x = F.one_hot(s, num_classes=self.num_tokens + 1)[:, :, :-1].to(
                torch.float32
            )
        ...
        r = (
            torch.tensor(sum(input_batch["traj_rewards"], []))
            .to(self.device)
            .clamp(min=0)
        )
    ...
    def forward(self, x, lens, return_all=False, coef=1, pad=2):
        if self.args.gen_model_type == "mlp":
            inp_x = F.one_hot(x, num_classes=self.num_tokens + 1)[:, :, :-1].to(
                torch.float32
            )
            ...
            out = (
                self.model(inp, None, lens=lens, return_all=return_all) * self.out_coef
            )
            return out
        out = (
            self.model(x.swapaxes(0, 1), x.eq(pad), lens=lens, return_all=return_all)
            * self.out_coef
        )
        return out
    ...
    def get_loss(self, batch):
        ...
        if self.args.gen_model_type == "mlp":
            ...
            pol_logits = self.logsoftmax2(
                self.model(x, None, return_all=True, lens=lens)
            )[:-1]
        ...
        seq_logits = (
            pol_logits.reshape((n, self.num_tokens))[
                torch.arange(n, device=self.device),
                (s[1:,].reshape((-1,))).clamp(0, self.num_tokens - 1),
            ].reshape(s[1:].shape)
            * mask[:, 1:].swapaxes(0, 1).logical_not().float()
        ).sum(0)
        # p(x) = R/Z <=> log p(x) = log(R) - log(Z) <=> log p(x) - log(Z)
        loss = (
            (self.model.Z + seq_logits - r.clamp(min=self.reward_exp_min).log())
            .pow(2)
            .mean()
        )
